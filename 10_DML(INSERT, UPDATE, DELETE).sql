/*      <       목   차      >
-- 참고 : 롤백 " 업데이트에 오류가 발생할 때, 이전 상태로 되돌리는 것
    1. DML : 데이터 조작언어
    2. INSERT : 테이블에 새로운 행을 추가하는 구문
    3. UPDATE : 테이블에 기록된 데이터를 수정하는 구문
    4. DELETE :  테이블에 기록 된 데이터를 삭제하는 구문
    5. TRUNCATE : 테이블의 전체 행을 삭제할 때 사용하는 구문, DELETE 보다 수행 속도가 더 빠르다. ROLLBACK이 불가능
