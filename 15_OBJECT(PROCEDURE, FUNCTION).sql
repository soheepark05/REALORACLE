...
/*
    <CURSOR>
        ...
        * 커서 속성 (묵시적 커서의 경우 커서명은 SQL로 사용된다.)
          - 커서명%NOTFOUND : 커서 영역에 남아있는 ROW 수가 없면 TRUE, 아니면 FALSE
...
BEGIN
    ...
    LOOP
        -- 서브 쿼리의 결과에서 한 행씩 자례대로 데이터를 가져온다.
...
/*
    <PACKAGE>
        프로시저와 함수르 효율적으로 관리하지 위해 묶는 단위로 패키지는 선언부, 본문(BODY)으로 나눠진다.
*/
