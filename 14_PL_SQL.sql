...
/*
    <PL/SQL>
        ...
        [PL/SQL의 구조]
            1) 선언부(DECLARE SECTION)         : DECLARE로 시작, 변수나 상수를 선언 및 초기화하는 부분이다. 디클리어
...
BEGIN
    ...
    INTO EID, ENAME, SAL
    FROM EMPLOYEE
    WHERE EMP_NAME = '&NAME'; -- &(앰퍼샌드) 기호는 대체 변수(값을 입력)를 입력하기 위한 창을 띄어주는 구문이다.
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('국가 코드 : ' || NCODE);
...
/*
        4) CASE 구문 
          [표현법]
            ...
            END;
*/
-- 사번을 입력받은 후에 사원의 모든 컬럼 데이이터를 EMP에 대입하고 DEPT_CODE에 따라 알맞는 부서를 출력한다.
DECLARE
    EMP EMPLOYEE%ROWTYPE;
    DNAME VARCHAR2(30);
BEGIN
