...
/*
    ...
    <컬럼값을 통한 산술 연산>
    SELECT절에 컬럼명 입력 부분에서 <<산술연산자>>를 사용해서 결과를 조회할 수 있다..
    */
...
/*
    <<컬럼명에 별칭 지정하기>>
    [표현법]
      1.  컬럼 AS 별칭 / 2. 컬럼 AS "별칭" / 3. 컬럼 별칭 / 4. 컬럼 "별칭"
...
----------EMPLOYEE 테이블에서 // 부서 코드(DEPT_CODE)가 D9와 일치하는 사원들의 모든 컬럼 조회
SELECT * FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; /*동등비교 == 아님*/

-- EMPLOYEE 테이블에서 부서 코드가 D9가 아닌 사원들의 (사번, 사원명, 부서 코드)만 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
--WHERE DEPT_CODE != 'D9';  /* != : 같지않다? */
--WHERE DEPT_CODE ^= 'D9'; /* ^= : 같지않다. */
WHERE DEPT_CODE <> 'D9'; /* <>: 같지않다.*/
...
-- EMPLOYEE 테이블에서 직급 코드가 J2이면서 급여가 400만원 이상인 직원들의 모든 컬럼 조회
SELECT * 
FROM EMPLOYEE
WHERE JOB_CODE = 'J2' OR SALARY >= 4000000;

-- EMPLOYEE 테이블에서 급여가 350만원 이상 600만원 이하를 받는 직원의 사번, 직원명, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
/* 
    ...
    - 비교대상컬럼 값이 하한값 이상이고, 상한값 이하인 경우 TRUE 리턴한다.
...
-- EMPLOYEE 테이블에서 성이 전씨인 사원 명, 급여 입사일 조회
SELECT EMP_NAME,SALARY, HIRE_DATE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '전%';

-- EMPLOYEE 테이블에서 이름중에 '하'가 포함된 사원명, 주민번호, 부서 코드 조회
...
-- EMPLOYEE 테이블에서 이메일 중 _ 앞글자가 3자리인 이메일 주소를 가진 사원의 사번, 사원명, 이메일 조회
-- EX) SUN_DIST@KH.OR.KR .....
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
--WHERE EMAIL LIKE '____%'; --와일드카드와 데이터 값이 구분되지 않는다,
--WHERE EMAIL LIKE '____\_%' ESCAPE '\'; --데이터로 처리할 값 앞에 임의의 문자를 제시하고 임의의 문자를 ESCAPE옵션에 등록한다.
WHERE EMAIL LIKE '___$_%' ESCAPE '$';

--EMPLOYEE 테이블에서 김씨 성이 아닌 직원의 사번, 사원명, 입사일 조회
SELECT EMP_ID, EMP_NAME, HIDE_DATE
FROM EMPLOYEE
...
--3. DEPRAPTMENT 테이블에서 해외영업부에 대한 모든 컬럼 조회
